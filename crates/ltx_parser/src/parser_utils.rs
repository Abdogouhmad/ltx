//! an expension of ltx parser utils to keep the code clean
use crate::LtxParser;

use ltx_lexer::{LtxToken, LtxTokenKind};

/// Utils impl for ltx parser
impl<'util> LtxParser<'util> {
    /// Peek at the current token kind without consuming it.
    #[inline]
    #[must_use]
    pub fn peek_kind(&self) -> Option<&LtxTokenKind<'util>> {
    ...
    #[inline]
    #[must_use]
    pub fn peek_at(&self, n: usize) -> Option<&LtxToken<'util>> {
        self.stream.peek_at(n)
    }

    /// Consume and return the current token.
    #[inline]
    pub fn bump(&mut self) -> Option<&LtxToken<'util>> {
        self.stream.bump()
    }
