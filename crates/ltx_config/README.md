...
**Configuration and project scaffolding for the `ltx` CLI.** Provides the data
model for `ltx.toml`, manifest validation, and project generation.
...
Validation errors embed the raw source text and a byte span so they render with
miette snippets pointing at the offending table or key.
