...
fn bench_catcode_get(c: &mut Criterion) {
    let state = CatCodeState::default_tex();
...
fn bench_catcode_set(c: &mut Criterion) {
    ...
    c.bench_function("catcode_set", |b| {
        b.iter(|| {
            let mut state = CatCodeState::default_tex();
            for &ch in &chars {
                state.set(ch, CatCode::Letter);
                black_box(&state);
            }
        })
    });
...
fn bench_catcode_get_vs_hashmap(c: &mut Criterion) {
    ...
    let array_state = CatCodeState::default_tex();

    // HashMap-based (for comparison)
    let mut hashmap_state = HashMap::new();
    // Initialize with same values
    for c in 32..=126 {
        hashmap_state.insert(c as u8 as char, CatCode::Other);
    }
    hashmap_state.insert('\\', CatCode::Escape);
    hashmap_state.insert('{', CatCode::BeginGroup);
    hashmap_state.insert('}', CatCode::EndGroup);
    hashmap_state.insert('$', CatCode::MathShift);
    hashmap_state.insert('&', CatCode::AlignmentTab);
    hashmap_state.insert('\n', CatCode::EndOfLine);
    hashmap_state.insert('#', CatCode::Parameter);
    hashmap_state.insert('^', CatCode::Superscript);
    hashmap_state.insert('_', CatCode::Subscript);
    hashmap_state.insert(' ', CatCode::Space);
    hashmap_state.insert('%', CatCode::Comment);
    hashmap_state.insert('~', CatCode::Active);
    for c in 'A'..='Z' {
        hashmap_state.insert(c, CatCode::Letter);
    }
    for c in 'a'..='z' {
        hashmap_state.insert(c, CatCode::Letter);
    }
    ...
    c.bench_function("catcode_get_hashmap", |b| {
        b.iter(|| {
            let mut sum = 0u8;
            for &ch in &chars {
                let cat = *hashmap_state.get(&ch).unwrap_or(&CatCode::Other);
                sum += cat as u8;
            }
            black_box(sum);
        })
    ...
    group.bench_function("set_1000_chars", |b| {
        b.iter(|| {
            ...
            for (i, &ch) in chars.iter().enumerate() {
                state.set(ch, if i % 2 == 0 { CatCode::Letter } else { CatCode::Other });
            }
            black_box(state);
        })
    });
...
fn bench_catcode_mixed_workload(c: &mut Criterion) {
    let test_cases = vec![
        ...
        ("with_commands", r"\section{Introduction} \textbf{bold} \textit{italic}"),
        ("with_math", r"$E=mc^2$ and $x^2 + y^2 = z^2$"),
        ("mixed", r"\documentclass{article} \begin{document} Hello $E=mc^2$ % comment \end{document}"),
    ];
...
fn bench_catcode_from_u8(c: &mut Criterion) {
    ...
    c.bench_function("catcode_from_u8", |b| {
        b.iter(|| {
            ...
            for &v in &values {
                if let Some(cat) = CatCode::from_u8(v) {
                    cats.push(cat);
                }
            }
            black_box(cats);
