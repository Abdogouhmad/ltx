...
**Compilation orchestration.** Turns a validated manifest into a compiled PDF by
dispatching to a configured engine. Currently only the `tectonic` engine is wired
up; the other engines emit a warning.
...
`miette::Report` and can be returned directly from `miette::Result` functions.
